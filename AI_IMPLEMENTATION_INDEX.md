...
- Railway now has both a repo-root and an `apps/api`-local `railway.json` so the api service can boot the compiled API entrypoint from either source root with Node specifier resolution enabled.
