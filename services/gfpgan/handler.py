"""RunPod Serverless handler for GFPGAN face restoration."""
...
def handler(job):
    ...
    fidelity = float(job_input.get("fidelity", os.environ.get("GFPGAN_FIDELITY", "0.7")))
    sharpen = float(job_input.get("sharpen", os.environ.get("GFPGAN_SHARPEN", "0.6")))
    try:
        result_bytes, result_type, result_name = _process_upload(raw_bytes, content_type, file_name, fidelity, sharpen)
