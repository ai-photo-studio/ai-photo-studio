"""RunPod Serverless handler for DDColor colorization."""
...
def handler(job):
    ...
    temperature = float(job_input.get("temperature", os.environ.get("DDCOLOR_TEMPERATURE", "0.55")))
    saturation = float(job_input.get("saturation", os.environ.get("DDCOLOR_SATURATION", "0.6")))
    try:
        result_bytes, result_type, result_name = _process_upload(raw_bytes, content_type, file_name, temperature, saturation)
