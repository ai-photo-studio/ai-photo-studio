...
export class WhatsAppImageFlowService {
  ...
  private readonly phase1ReplicateOnly: boolean;

  constructor(private readonly config: AppConfig) {
    this.phase1ReplicateOnly = String(config.PHASE1_REPLICATE_ONLY || "").toLowerCase() === "true";
  ...
  async handleIncomingImage(from: string, mediaId: string) {
    if (this.phase1ReplicateOnly) {
      throw new AppError("Phase 1 runtime does not allow WhatsApp AI processing paths", 503, "PHASE1_REPLICATE_ONLY");
    }
