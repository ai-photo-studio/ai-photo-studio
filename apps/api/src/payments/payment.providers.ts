...
export class DemoPaymentProvider extends BasePaymentProvider {
  ...
  async createCheckout(input: PaymentCheckoutInput): Promise<PaymentCheckoutResult> {
    const providerRef = `DEMO-${input.orderNo}-${Date.now().toString(36).toUpperCase()}`;
    const checkoutUrl = `http://localhost:4000/demo-payment?orderNo=${input.orderNo}&ref=${providerRef}`;
    return {
      providerName: "demo",
      providerRef,
      checkoutUrl,
      instructions: "Demo payment mode - auto approved",
