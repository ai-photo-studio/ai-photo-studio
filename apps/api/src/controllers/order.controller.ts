...
import { readGuestOwnershipToken, matchesGuestOwnershipToken } from "../utils/guest-ownership";
...
export class OrderController {
  ...
  getOrder = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const canAccess = this.canAccessOrder(result, user?.sub, guestToken);
      const downloadAllowed = Boolean(result.processedUrl && canAccess);
  ...
  uploadWebImage = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      } else if (!matchesGuestOwnershipToken(order.guestOwnershipTokenHash, guestToken)) {
  ...
  createOrderCheckout = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const isDemo = this.config.paymentProvider === "demo";
      ...
      await this.paymentService.createPaymentRecord({
        ...
        status: isDemo ? "APPROVED" : undefined
      });

      // In demo mode, auto-approve the payment and process the order
      if (isDemo) {
        const payment = await prisma.payment.findFirst({
          where: { orderId: order.id, provider: "demo" },
          orderBy: { createdAt: "desc" }
        });
        if (payment) {
          await this.paymentService.approvePaymentById(payment.id, "system");
        }
      }
  ...
  private canAccessOrder(
    order: { userId: string | null; guestOwnershipTokenHash?: string | null },
    ...
    if (guestToken) {
      return matchesGuestOwnershipToken(order.guestOwnershipTokenHash, guestToken);
    }
    return Boolean(userId && !order.userId);
  }
