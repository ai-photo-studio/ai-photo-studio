export const validateCanaryBudget = (input: { approved?: boolean; endpointId?: string; templateId?: string; gpuType?: string; ratePerSecond?: number | null; budget?: number | null; timeoutSeconds?: number; maximumJobs?: number; retries?: number; concurrency?: number; productionRoutingAllowed?: boolean; immutableImageDigest?: string; expectedDigest?: string; sourceSha?: string; expectedSourceSha?: string }) => {
  ...
  if (!input.gpuType?.trim()) throw new Error("gpu type is required");
  ...
  if ((input.sourceSha || "") !== (input.expectedSourceSha || "")) throw new Error("source SHA mismatch");
  return true;
};
