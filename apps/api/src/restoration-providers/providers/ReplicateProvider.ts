...
export class ReplicateProvider implements IRestorationProvider {
  ...
  private async createPrediction(base64Image: string, contentType: string, upscaleScale?: number): Promise<ReplicatePrediction> {
    ...
    const url = `${REPLICATE_API_BASE}/models/${MODEL_OWNER}/${MODEL_NAME}/versions/${MODEL_VERSION}/predictions`;
    logger.info("E2E: createPrediction START", { url, model: `${MODEL_OWNER}/${MODEL_NAME}`, version: MODEL_VERSION });
    ...
    logger.info("E2E: createPrediction HTTP response", { status: response.status, statusText: response.statusText });

    if (!response.ok) {
      const body = await response.text();
      logger.error("E2E: Replicate API request failed", { status: response.status, body: body.slice(0, 300) });
      throw new Error(`Replicate API failed (${response.status}): ${body.slice(0, 200)}`);
    }

    const prediction = (await response.json()) as ReplicatePrediction;
    logger.info("E2E: createPrediction END", { predictionId: prediction.id, status: prediction.status, output: prediction.output ? "present" : "null", error: prediction.error });
    return prediction;
  }

  private async pollPrediction(predictionId: string): Promise<ReplicatePrediction> {
    const deadline = Date.now() + this.maxPollTimeMs;
    logger.info("E2E: pollPrediction START", { predictionId, maxPollMs: this.maxPollTimeMs });
    ...
    while (Date.now() < deadline) {
      ...
      if (prediction.status === "succeeded") {
        logger.info("E2E: pollPrediction succeeded", { predictionId, output: prediction.output ? "present" : "null" });
        return prediction;
      }

      if (prediction.status === "failed") {
        logger.error("E2E: pollPrediction failed", { predictionId, error: prediction.error });
    ...
    logger.error("E2E: pollPrediction timed out", { predictionId, maxPollMs: this.maxPollTimeMs });
  ...
  private async handleResult(prediction: ReplicatePrediction, startTime: number, request: RestorationRequest): Promise<RestorationResult> {
    ...
    logger.info("E2E: downloading result image", { outputUrl: outputUrl.substring(0, 80) + "..." });
    ...
    logger.info("E2E: image downloaded", { sizeBytes: outputBuffer.length, contentType: imgResponse.headers.get("content-type") });
