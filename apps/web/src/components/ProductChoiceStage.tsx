...
export default function ProductChoiceStage({ selected, onSelect, onContinue, busy = false, error = null }: ProductChoiceStageProps) {
  return <main className="tn-product-page">
    ...
    <section className="tn-product-options" aria-label="Choose a ThanNow product">
      <button type="button" role="radio" aria-checked={selected === "digital"} className={`tn-product-card tn-product-card--digital${selected === "digital" ? " is-selected" : ""}`} onClick={() => onSelect("digital")} aria-pressed={selected === "digital"}>
        <span className="tn-product-card__top"><span className="tn-product-card__title-group"><span className="tn-product-card__round-icon tn-product-card__round-icon--green"><Icon name="download" size={31} /></span><strong className="tn-product-card__title">Digital Download</strong></span><span className={`tn-product-selected${selected === "digital" ? " is-visible" : ""}`}><Icon name="check" size={28} /></span></span>
        <span className="tn-product-card__body tn-product-card__body--digital"><span className="tn-product-card__copy">A high-resolution digital file, ready to download and keep private.</span><img className="tn-product-card__art" src="/thannow/product-choice/digital-devices.webp" alt="Pakistani family photograph displayed on a mobile phone, tablet and laptop" loading="eager" decoding="async" /></span>
        <span className="tn-product-features tn-product-features--green"><Feature icon="cloud">High-resolution<br />file</Feature><Feature icon="infinity">Download<br />anytime</Feature><Feature icon="shield">Secure and<br />private</Feature></span>
      </button>
      <button type="button" role="radio" aria-checked={selected === "print"} className={`tn-product-card tn-product-card--print${selected === "print" ? " is-selected" : ""}`} onClick={() => onSelect("print")} aria-pressed={selected === "print"}>
        <span className="tn-product-card__top"><span className="tn-product-card__title-group"><span className="tn-product-card__round-icon tn-product-card__round-icon--purple"><Icon name="printer" size={31} /></span><strong className="tn-product-card__title">Print + Digital<br />Home Delivery</strong></span><span className={`tn-product-selected tn-product-selected--purple${selected === "print" ? " is-visible" : ""}`}><Icon name="check" size={28} /></span></span>
        <span className="tn-product-card__body tn-product-card__body--print"><span className="tn-product-card__copy">A premium physical print plus a digital copy, delivered to your door.</span><img className="tn-product-card__art tn-product-card__art--print" src="/thannow/product-choice/print-home-delivery.webp" alt="Pakistani family photograph with a printer, frames and home delivery box" loading="eager" decoding="async" /></span>
        <span className="tn-product-features tn-product-features--purple"><Feature icon="diamond">Premium print<br />quality</Feature><Feature icon="truck">Delivered to<br />your door</Feature><Feature icon="cloud">Digital copy<br />included</Feature><Feature icon="shield">Safe delivery</Feature></span>
      </button>
    </section>
