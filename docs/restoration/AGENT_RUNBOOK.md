...
GFPGANv1.4 weight provenance is established by the `gfpgan-weight-provenance.yml` workflow (run 30618746285). The official asset is 348632874 bytes; SHA-256 e2cd4703ab14f4d01fd1383a8a8b266f9a5833dacee8e6a79d3bf21a1b6be5ad; checksum source independently-calculated because the release API digest is absent. Apache-2.0 covers source code only and does not grant weight redistribution. Do not commit or bundle model weights; use externally-mounted weights that match the pinned checksum before startup; never download weights at runtime. A separate unpublished GFPGAN GPU candidate exists at `apps/api/runpod-worker-gpu-dev/` for build and contract tests only; it mounts the weight externally and fails closed on size/checksum mismatch or missing CUDA. Its build-only CI passed (run 30620758562, image ID sha256:bf6af925ca2d4e3ef9c877a5fcde84907f30ad917e17f0e5591ef907081a8846, PyTorch 2.1.2+cu121); health/gpu_probe/fail-closed-restore tests passed; GPU inference was not executed. Never bundle weights without explicit redistribution approval; never bypass checksum validation; never replace the verified checksum without new provenance evidence; build success is not GPU inference or quality approval. New Gate 2 approval is required before publishing any GPU image; Gate 3 and Gate 4 remain prohibited.
