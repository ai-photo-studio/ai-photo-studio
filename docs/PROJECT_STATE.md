...
## Updated 2026-08-07 (R9.3-P10 hero quality fix — implemented, deploy NOT executed)
- **Display:** hero comparison switched from `object-fit: cover` (cropped) to `object-fit: contain` + `object-position: center` with a blurred/darkened same-image background; full photo always visible, Then/Now pixel-aligned at 1600x1600. Caption moved to a strip BELOW the frame; Upload CTA moved outside the photo area. Slider handle stays in-frame.
- **10 damage presets:** all `*-then.jpg` regenerated deterministically from their exact matching `*-now.jpg` via seeded, idempotent generator `apps/web/scripts/generate-hero-then.cjs` (sharp). Each hero uses a distinct realistic damage style (verified: 10 distinct hashes, all 1600x1600). Then/Now remain the exact same scene/people/composition.
- **Assets:** all 20 files (10 then + 10 now) resolve 1600x1600, packaged into `dist/assets/hero/hero/` on build.
- **Tests:** `typecheck` PASS, `build` PASS, `lint` 0 errors, `test:browser:responsive` 18/18 PASS, full `test:browser` 59/59 PASS (added 5 hero-quality tests: contain/centered, identical geometry, blurred bg, caption/CTA outside photo, all 20 assets resolve). Deployment to production NOT executed — requires separate explicit owner authorization.

## Updated 2026-08-07 (R9.3-P11 hero-quality production deploy — PRODUCTION_LIVE)
- Owner-authorized push + deploy of hero-quality commit `72825bc9237039f54a4f2303d8c7f3665e78857e`. Pushed to `origin/setup/project-automation` (`9e40d13..72825bc`, local == remote).
- Production deploy via `npx wrangler pages deploy apps/web/dist --project-name ai-photo-studio-frontend --branch main` → **Production** deployment `3a52a1f0-8f14-4a9b-a5d0-704d52820d3a`, branch `main`, commit `72825bc`, target `https://www.thannow.com` (supersedes `abadf7d7`/cceb5d0).
- Live hero-quality smoke at `www.thannow.com`: CSS loaded correctly; full photo visible (`object-fit: contain`, center); one compare frame; blurred same-image background fills frame; sharp image fully contained; Then/Now pixel-aligned 1600x1600 + identical layer geometry; drag left=Then right=Now; mouse+touch+pointer; auto-rotation ~7s resets 50%; caption + Upload CTA outside image; CTA → `/restore/new`; no overflow at 1440/1024/768/430/390/360; all 20 assets HTTP 200; rotation cycles all 10 distinct damage presets; no launch-critical console/page/network errors. All PASS.
- **Hero stage: 100% PRODUCTION_LIVE.**

## Updated 2026-08-08 (R9.3-P10B hero desktop visual repair — verified locally, NOT pushed/deployed)
- Fixed desktop hero visual defects (ghost/double image, left-right cut, oversized tablet frame, vertical-triangle handle, confusing direction, artificial damage overlays). Removed the blurred `.hero-bg` ghost (square frame fully fits the 1600x1600 source); frame capped `max-width: 620px` + centered (uniform full-image presentation at 1440/1280/1024/768, full column on mobile 430/390/360); handle is now a horizontal `< >` double-arrow; then/now labels added as small UI/CSS pills left/right; all 10 `*-then.jpg` regenerated photorealistically (grain, blurred mottle/stains, fine scratches, feathered torn edges/missing corners — no geometric circles/polygons/fake-UI/transparency).
- Verified at all 6 viewports: exactly one Then + one Now layer, no `.hero-bg`, Then inside frame (no crop/overflow), horizontal arrow handle, no horizontal overflow. Tests: `typecheck` ✓ `build` ✓ `lint` 0 errors ✓ `test:browser:responsive` 18/18 ✓ `test:browser` 62/62 ✓. All 20 assets 1600x1600 in `dist/`.
- R9.4-P2 and R9.3-P11 held (no push/deploy).

## Updated 2026-08-08 (R9.3-P12 hero visual-QA production deploy — PRODUCTION_LIVE)
- Owner-authorized push + deploy of hero repair commit `646f27b565aa7b9fb7baa6a781f8656ccf8c2662`. Pushed `568d74a..646f27b` to `origin/setup/project-automation` (local == remote). Production deploy via `npx wrangler pages deploy apps/web/dist --project-name ai-photo-studio-frontend --branch main` → **Production** deployment `50ea593f-8ecc-44c7-83ab-ea1cbd7e0a8d` (branch `main`, commit `646f27b`), target `https://www.thannow.com` (supersedes `3a52a1f0`).
- Live smoke at `www.thannow.com` all PASS: one compare frame; exactly one Then + one Now (no ghost background); full image inside square frame (`object-fit: contain`, no left/right crop); Then/Now UI labels left/right; horizontal `< >` arrow handle + `touch-action:none`; drag left=Then right=Now; random first hero (4 distinct across 5 loads); ~7s rotation resets 50%; caption + Upload CTA below photo (CTA isolated-measured below frame at 1440/1024/768); floating mobile CTA visible at 430/390/360; no overflow at 1440/1024/768/430/390/360; all 20 assets 200; no `hero-compare.png`; no launch-critical console/page/network errors.
- **Hero stage: 100% PRODUCTION_LIVE (repaired desktop geometry + photorealistic damage).** R9.4-P2 and Bank Alfalah/payment remain HELD for GPT-5.6 Sol.
