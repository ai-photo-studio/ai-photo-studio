...
- API:
  ...
  - `apps/api/railway.json` exists so a Railway service rooted at `apps/api` can boot the compiled API entrypoint without monorepo path drift
...
- Phase I also adds a service-local Railway config under `apps/api` to remove ambiguity if the Railway service root is set to the API workspace instead of the repository root, and both Railway configs now boot the compiled API artifact with Node specifier resolution enabled.
